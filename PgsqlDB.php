...
class PgsqlDB extends PdoDB
{
    public function __construct($id)
    {
        ...
        if (DB_ALLOW_CREATE_TABLES) {
            $stmt = $this->db->prepare('SELECT COUNT(*) FROM information_schema.tables WHERE table_name = \'' . $this->prefix . 'LAYOUT\'');
            $stmt->execute([]);
            if ($stmt->fetchColumn() === 0) {
                $this->db->exec('CREATE TABLE IF NOT EXISTS "' . $this->prefix . 'LAYOUT" (time bigint NOT NULL UNIQUE, value text NOT NULL);');
                $this->db->exec('CREATE TABLE IF NOT EXISTS "' . $this->prefix . 'EVENT" (time bigint NOT NULL UNIQUE, name varchar(256) NOT NULL, color varchar(7) NOT NULL, "end" bigint);');
                $this->db->exec('CREATE TABLE IF NOT EXISTS "' . $this->prefix . 'INFO" (time bigint NOT NULL UNIQUE, info text NOT NULL);');
                $this->db->exec('CREATE TABLE IF NOT EXISTS "' . $this->prefix . 'INVOICE" ("key" varchar(256) NOT NULL UNIQUE, value text NOT NULL);');
                $this->db->exec('CREATE TABLE IF NOT EXISTS "' . $this->prefix . 'NOTE" ("key" varchar(256) NOT NULL UNIQUE, value text NOT NULL);');
            }
        }
    }
